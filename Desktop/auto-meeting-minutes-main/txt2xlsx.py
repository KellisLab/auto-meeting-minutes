...
def txt_to_xlsx(input_file, output_file, debug=True):
    """
    ...
    """
    if debug:
        print(f"\n=== DEBUGGING TXT TO XLSX CONVERSION ===")
    ...
    if debug:
        print(f"Total file content length: {len(content)} characters")
        ...
        # Show first few lines for format inspection
        print(f"\nFirst 5 non-empty lines:")
        for i, line in enumerate(non_empty_lines[:5]):
            print(f"  {i+1}: '{line}'")
    
    # IMPROVED REGEX PATTERNS - Try multiple patterns to catch more entries
    patterns = [
        # Original strict pattern
        r'(\d{2}:\d{2}:\d{2}) ([^:]+): (.+)',
        # Pattern with optional milliseconds and flexible spacing
        r'(\d{2}:\d{2}:\d{2}(?:\.\d{3})?)\s+([^:]+?):\s*(.+)',
        # Pattern with brackets around timestamp
        r'\[(\d{2}:\d{2}:\d{2})\]\s*([^:]+?):\s*(.+)',
        # Pattern with more flexible spacing around colon
        r'(\d{2}:\d{2}:\d{2})\s*([^:]+?)\s*:\s*(.+)',
        # Pattern allowing for tabs or multiple spaces
        r'(\d{2}:\d{2}:\d{2})[\s\t]+([^:]+?)[\s\t]*:[\s\t]*(.+)',
        # Pattern for lines that might have extra whitespace
        r'^\s*(\d{2}:\d{2}:\d{2})\s+([^:]+?):\s*(.+)$',
    ]
    ...
    # If no pattern worked, try line-by-line approach
    if not best_matches:
        if debug:
            print("\n⚠️ No regex patterns worked, trying line-by-line parsing...")
        ...
        for line in lines:
            ...
            if timestamp_match:
                ...
                remainder = line[len(timestamp):].strip()
                
                # Look for speaker and text
                if ':' in remainder:
                    # Find the first colon that's likely the speaker separator
                    colon_pos = remainder.find(':')
                    speaker = remainder[:colon_pos].strip()
                    text = remainder[colon_pos+1:].strip()
                    
                    if speaker and text:
                        line_matches.append((timestamp, speaker, text))
        
        if line_matches:
            best_matches = line_matches
            if debug:
                print(f"✅ Line-by-line parsing found {len(line_matches)} matches")
    
    if not best_matches:
        print("\n❌ ERROR: No matches found with any pattern!")
        if debug:
            print("Sample content to analyze format:")
            print(content[:1000])
        return output_file
    ...
    if debug:
        ...
        if gaps:
            print(f"\n⚠️ FOUND {len(gaps)} SIGNIFICANT TIME GAPS (>30s):")
            for gap_start, gap_end, gap_size in gaps[:5]:  # Show first 5
                print(f"  Gap: {gap_start} → {gap_end} ({gap_size:.1f}s)")
            if len(gaps) > 5:
    ...
    if debug:
        print(f"Found {len(all_speakers)} unique speakers: {sorted(list(all_speakers))}")
    ...
    if debug:
        print(f"✅ Processed {len(data)} transcript entries")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python txt2xlsx.py input.txt [output.xlsx] [--no-debug]")
    ...
    txt_to_xlsx(input_file, output_file, debug=debug)