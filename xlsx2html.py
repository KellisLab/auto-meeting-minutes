...
5. Generates a separate meeting_summaries.html file with batch summaries (40-min chunks)
...
# -------------------------------------------------------------
# NEW: Meeting Summary Batch Processing
# -------------------------------------------------------------
...
def summarize_batch(batch_text, batch_number, start_time, end_time, api_key):
    """
    Summarize a batch of transcript text using OpenAI API
    """
    if not api_key:
    ...
    try:
        ...
        # Construct prompt for batch summary
        prompt = (
            ...
            "Numbered full paragraphs with bolded important words, and in-line headers (but no line breaks). "
        ...
        response = openai.chat.completions.create(
            ...
            max_tokens=500,  # More tokens for batch summaries
        )
...
def generate_meeting_summaries_html(batches, batch_summaries, video_id, html_file):
    """
    Generate HTML file with meeting batch summaries
    """
    html_lines = []
...
def summarize_segment(segment_text, speaker, start_time, end_time, api_key):
    ...
    try:
        ...
        # Construct prompt
        prompt = (
            "Summarize this meeting notes for the speaker"
            "start with speaker name, then -key topic- then summary (no line breaks)"
            "example format: Jerry Lu – Optimization of UMAP and JSON Communication: Jerry Lu presented specific..."
...
def process_xlsx(xlsx_file, video_id, html_file=None, format_type='simple', summary_file=None):
    ...
    if html_file is None:
        html_file = os.path.splitext(xlsx_file)[0] + '.speaker_summaries.html'
    ...
    try:
        ...
        # NEW: Create batches for meeting summaries
        ...
        # Generate meeting summaries HTML
...
def main():
    ...
    parser.add_argument('--summary-file', help='Output file for meeting summaries (optional)')
   
    args = parser.parse_args()
   
    try:
        html_file, summary_file = process_xlsx(
            args.input_file,
            args.video_id,
            args.output_file,
            args.format,
